...
func HeadroomFactory(name string, rawParameters json.RawMessage, _ framework.Handle) (framework.BBRPlugin, error) {
	...
	p, err := NewHeadroomPlugin(config.HeadroomURL, config.TimeoutSeconds, config.isFailOpen())
	if err != nil {
...
func NewHeadroomPlugin(headroomURL string, timeoutSeconds int, failOpen bool) (*HeadroomPlugin, error) {
	client, err := newHeadroomClient(headroomURL, timeoutSeconds)
	if err != nil {
		return nil, err
	}
...
func (p *HeadroomPlugin) ProcessRequest(ctx context.Context, cycleState *framework.CycleState, request *framework.InferenceRequest) error {
	...
	if result.TokensSaved <= 0 {
		logger.Info("headroom: no compression savings")
		return nil
	}
